...
- Auth backend: embedded `tsnet` node vs the host's already-running `tailscaled` local API — **resolved 2026-05-10 (M3.0), amended 2026-05-12:** v1.0 first consults the host's `tailscaled` local API via Unix socket when available, then falls back to `tailscale whois --json` for macOS App Store / GUI installs that expose the CLI but no local socket. Both paths resolve `WhoIs(peerAddr)` into the device's tailnet identity. The relay binds `0.0.0.0:4399` and relies on Tailscale ACLs / Funnel-off to keep the public out. Embedded `tsnet` is revisited in v1.2 only if Tailscale ACL push notifications become required.
