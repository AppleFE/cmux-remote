...
# launchd 데몬(root)으로 빌드 + 설치 — 포트 80 바인딩에 sudo 필요
# 스크립트가 swift build -c release를 알아서 실행합니다.
sudo ./scripts/install-launchd.sh
```

설치 스크립트는 릴리스 바이너리를 빌드해 `/usr/local/lib/cmux-remote/bin/`로 복사하고,
`~/.cmuxremote/relay.json` 기본 설정이 없으면 자동 생성하며,
`/Library/LaunchDaemons/com.genie.cmuxremote.plist`를 렌더링한 뒤
서비스를 부트스트랩합니다. 데몬은 root로 `0.0.0.0:80`에 바인딩하며, 로그는 `~/.cmuxremote/log/`로 떨어집니다.
...
`starting cmux-relay on 0.0.0.0:80` → `listening …` →
...
1. **Add Mac** 탭
2. 위에서 확인한 Tailscale IP 또는 MagicDNS 이름 입력, 포트는 `80`
...
디바이스별 토큰이 발급되며
`/usr/local/lib/cmux-remote/bin/cmux-relay devices revoke <id>`로 언제든 해지할 수 있습니다.
...
```bash
sudo ./scripts/install-launchd.sh            # swift build -c release 포함
sudo ./scripts/uninstall-launchd.sh          # bootout + plist 제거
```

정상 기동 시 `stderr.log`에 `starting cmux-relay on 0.0.0.0:80` →
...
| 확인 | 명령 | 안 되면 |
|---|---|---|
| ① cmux 실행 중? | `cmux --version` | cmux 앱을 켜고 → `sudo launchctl kickstart -k "$SERVICE"` |
| ② relay 떠 있나? | `curl -s http://$(tailscale ip -4):80/v1/health` | `sudo launchctl kickstart -k "$SERVICE"`, 그래도 안 되면 `sudo ./scripts/install-launchd.sh` 재실행 |
| ③ 로그 정상? | `tail -n 40 ~/.cmuxremote/log/stderr.log` | 아래 로그별 대응 참고 |
| ④ Tailscale 양쪽 온라인? | `tailscale status` | Mac·iPhone 둘 다 같은 Tailnet 로그인 확인 |
| ⑤ 앱 주소 맞나? | `tailscale ip -4` | 앱에 이 IP + 포트 `80` 입력했는지 확인 |

로그별 대응:

- `cmux event stream unavailable: socketMissing` — **cmux가 꺼져 있음.**
  cmux 앱을 켜고 `sudo launchctl kickstart -k "$SERVICE"`.
- `Connection refused` 반복 — **소켓 경로가 바뀜**(cmux 재시작으로 이름이
  바뀌었거나, 업데이트가 소켓을 `~/.local/state/cmux`로 옮김). 최신 relay는
  마커를 자동 추적하니 `sudo ./scripts/install-launchd.sh`로 재설치하면 해결.
  급하면 `cat /tmp/cmux-last-socket-path`의 경로를 `CMUX_SOCKET_PATH`로 핀.
- 헬스 체크는 OK인데 앱만 못 붙음 — **네트워크/주소 문제.** iPhone과
  Mac이 같은 Tailnet인지, 앱 주소·포트(`80`)가 맞는지, 디바이스 토큰이
  revoke되지 않았는지(`/usr/local/lib/cmux-remote/bin/cmux-relay devices list`) 확인.

> cmux를 자주 재시작한다면, 소켓 회전 후 relay를 다시 붙이는 가장 빠른
> 방법은 `sudo launchctl kickstart -k "$SERVICE"` 입니다.
